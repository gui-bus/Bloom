...
export default function TabsComponentPage() {
  ...
  const [closableTabs, setClosableTabs] = React.useState([
    ...
    { id: "tab2", title: "Settings", content: "Application preferences and configuration." },
    { id: "tab3", title: "Analytics", content: "Traffic reports and performance metrics." },
  ]);
  ...
  const handleAddTab = () => {
    ...
    setDynamicTabs([
      ...dynamicTabs,
      { id: newId, title: `Tab ${nextNum}`, content: `Content panel for Tab ${nextNum}` },
    ]);
  };
  ...
  const handleSaveRename = (id: string) => {
    if (editingTitle.trim()) {
      setDynamicTabs(
        dynamicTabs.map((t) => (t.id === id ? { ...t, title: editingTitle.trim() } : t))
      );
    }
  ...
  return (
    <div className="space-y-8">
      ...
      <ImportSnippet importCode={`import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/ui/tabs/tabs";`} />
        ...
        preview={
          <div className="w-full">
            <Tabs defaultValue={closableTabs[0]?.id || "tab1"}>
              ...
              {closableTabs.map((t) => (
                <TabsContent key={t.id} value={t.id} className="p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 mt-3">
                ...
                {dynamicTabs.map((t) => (
                  <TabsTrigger key={t.id} value={t.id}>
                    {editingTabId === t.id ? (
                      ...
                      <div className="flex items-center gap-1.5" onDoubleClick={(e) => handleStartRename(e, t.id, t.title)}>
                        <span>{t.title}</span>
                        <span
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          <Icon icon="hugeicons:pencil-edit-01" className="size-3" />
                        </span>
                      </div>
                    )}
                  </TabsTrigger>
        ...
        preview={
          <div className="w-full max-w-xl">
            <Tabs defaultValue="v1" orientation="vertical" variant="vertical">
              <TabsList>
                <TabsTrigger value="v1" startContent={<Icon icon="hugeicons:user-02" />}>Profile</TabsTrigger>
                <TabsTrigger value="v2" startContent={<Icon icon="hugeicons:notification-01" />}>Notifications</TabsTrigger>
                <TabsTrigger value="v3" startContent={<Icon icon="hugeicons:security-check" />}>Privacy</TabsTrigger>
              </TabsList>
              <TabsContent value="v1" className="p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
                <h4 className="text-sm font-bold">Profile Details</h4>
                <p className="text-xs text-zinc-500 mt-1">Manage user avatar and preferences.</p>
              </TabsContent>
              <TabsContent value="v2" className="p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
                <h4 className="text-sm font-bold">Notification Channels</h4>
                <p className="text-xs text-zinc-500 mt-1">Configure email and push alert settings.</p>
              </TabsContent>
              <TabsContent value="v3" className="p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
                <h4 className="text-sm font-bold">Privacy Settings</h4>
                <p className="text-xs text-zinc-500 mt-1">Control data sharing preferences.</p>
              </TabsContent>
            </Tabs>
          </div>
        }
      ...
      <DocsComponent
        ...
        preview={
          <div className="flex flex-col gap-6 w-full">
            <div>
              <span className="text-xs text-zinc-500 dark:text-zinc-400 block mb-2 font-semibold">1. Default Variant</span>
              <Tabs defaultValue="def1">
                <TabsList>
                  <TabsTrigger value="def1" variant="default">Default 1</TabsTrigger>
                  <TabsTrigger value="def2" variant="default">Default 2</TabsTrigger>
                </TabsList>
              </Tabs>
            </div>

            <div>
              <span className="text-xs text-zinc-500 dark:text-zinc-400 block mb-2 font-semibold">2. Contained Variant</span>
              <Tabs defaultValue="c1">
                <TabsList>
                  <TabsTrigger value="c1" variant="contained">Contained 1</TabsTrigger>
                  <TabsTrigger value="c2" variant="contained">Contained 2</TabsTrigger>
                </TabsList>
              </Tabs>
            </div>

            <div>
              <span className="text-xs text-zinc-500 dark:text-zinc-400 block mb-2 font-semibold">3. Pills Variant</span>
              <Tabs defaultValue="p1">
                <TabsList>
                  <TabsTrigger value="p1" variant="pills">Pill 1</TabsTrigger>
                  <TabsTrigger value="p2" variant="pills">Pill 2</TabsTrigger>
                </TabsList>
              </Tabs>
            </div>

            <div>
              <span className="text-xs text-zinc-500 dark:text-zinc-400 block mb-2 font-semibold">4. Underlined Variant</span>
              <Tabs defaultValue="u1">
                <TabsList background={false}>
                  <TabsTrigger value="u1" variant="underlined" color="primary">Underline 1</TabsTrigger>
                  <TabsTrigger value="u2" variant="underlined" color="primary">Underline 2</TabsTrigger>
                </TabsList>
              </Tabs>
            </div>

            <div>
              <span className="text-xs text-zinc-500 dark:text-zinc-400 block mb-2 font-semibold">5. Bordered Variant</span>
              <Tabs defaultValue="b1">
                <TabsList background={false}>
                  <TabsTrigger value="b1" variant="bordered">Bordered 1</TabsTrigger>
                  <TabsTrigger value="b2" variant="bordered">Bordered 2</TabsTrigger>
                </TabsList>
              </Tabs>
            </div>
          </div>
...
<TabsTrigger variant="bordered">Bordered</TabsTrigger>`}
        props={["variant: 'default' | 'contained' | 'pills' | 'underlined' | 'bordered' | 'vertical'"]}
      />
              ...
              <tbody>
                <tr className="border-b border-border">
                  ...
                  <td className="px-3 py-2 font-mono text-xs text-muted-foreground">
                    'default' | 'contained' | 'pills' | 'underlined' | 'bordered' | 'vertical'
                  </td>
                  <td className="px-3 py-2 text-muted-foreground">'default'</td>
                  <td className="px-3 py-2 text-muted-foreground">Visual style of active tab trigger.</td>
                </tr>
                <tr className="border-b border-border">
                  <td className="px-3 py-2 font-mono text-primary">isClosable</td>
                  <td className="px-3 py-2 font-mono text-xs text-muted-foreground">boolean</td>
                  <td className="px-3 py-2 text-muted-foreground">false</td>
                  <td className="px-3 py-2 text-muted-foreground">Renders a close ('x') icon button on the trigger.</td>
                </tr>
                <tr className="border-b border-border">
                  <td className="px-3 py-2 font-mono text-primary">addable</td>
                  <td className="px-3 py-2 font-mono text-xs text-muted-foreground">boolean</td>
                  <td className="px-3 py-2 text-muted-foreground">false</td>
                  <td className="px-3 py-2 text-muted-foreground">Renders '+' button to dynamically append new tabs.</td>
                </tr>
                <tr className="border-b border-border">
                  <td className="px-3 py-2 font-mono text-primary">isScrollable</td>
                  <td className="px-3 py-2 font-mono text-xs text-muted-foreground">boolean</td>
                  <td className="px-3 py-2 text-muted-foreground">false</td>
                  <td className="px-3 py-2 text-muted-foreground">Renders left and right scroll navigation arrows.</td>
                </tr>
              </tbody>
            </table>
          </div>
