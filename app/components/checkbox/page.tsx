...
export default function CheckboxComponentPage() {
  ...
  const [componentsState, setComponentsState] = React.useState<
  ...
  const componentKeys = ["input", "select", "checkbox", "slider", "datepicker"];
  const checkedComponentsCount = componentKeys.filter(
    (k) => componentsState[k],
  ).length;

  const isIndeterminateAllSelected =
    checkedComponentsCount === componentKeys.length;
  const isIndeterminateSomeSelected =
    checkedComponentsCount > 0 && checkedComponentsCount < componentKeys.length;

  const handleToggleAllComponents = () => {
    const nextState = !isIndeterminateAllSelected;
    setComponentsState({
      input: nextState,
      select: nextState,
      checkbox: nextState,
      slider: nextState,
      datepicker: nextState,
    });
  };
  ...
  return (
    <div className="space-y-8">
      <DocsTitle
        ...
        description="An accessible set of selection controls supporting individual checkboxes, group management, grid layout columns, selectable card mode, and helper hooks."
      />

      <ImportSnippet
        importCode={`import { Checkbox, CheckboxGroup, useCheckboxGroup } from "@/components/ui/checkbox/checkbox";`}
      />
...
<Checkbox color="success" label="Success Theme" defaultChecked />`}
      ...
      <DocsComponent
        title="Group Select All / Deselect All Hook"
...
<div className="flex gap-2">
  <Button variant="flat" radius="sm" size="xs" onClick={selectAll}>Select All</Button>
  <Button variant="flat" radius="sm" size="xs" onClick={deselectAll}>Clear</Button>
</div>
      ...
      <DocsComponent
        ...
        preview={
          <div className="flex flex-col gap-2 w-full max-w-sm">
            <Checkbox
              checked={isIndeterminateAllSelected}
              isIndeterminate={isIndeterminateSomeSelected}
              onCheckedChange={handleToggleAllComponents}
              label="Select All Components"
              description={`${checkedComponentsCount} of ${componentKeys.length} components selected`}
            />
            <div className="ml-6 flex flex-col gap-2">
              <Checkbox
                label="Input"
                checked={componentsState.input}
                onCheckedChange={() => handleToggleComponent("input")}
              />
              <Checkbox
                label="Select"
                checked={componentsState.select}
                onCheckedChange={() => handleToggleComponent("select")}
              />
              <Checkbox
                label="Checkbox"
                checked={componentsState.checkbox}
                onCheckedChange={() => handleToggleComponent("checkbox")}
              />
              <Checkbox
                label="Slider"
                checked={componentsState.slider}
                onCheckedChange={() => handleToggleComponent("slider")}
              />
              <Checkbox
                label="DatePicker"
                checked={componentsState.datepicker}
                onCheckedChange={() => handleToggleComponent("datepicker")}
              />
            </div>
          </div>
        }
        code={`const [componentsState, setComponentsState] = React.useState({
  input: true,
  select: true,
  checkbox: true,
  slider: false,
...
/>`}
      ...
      <DocsComponent
        title="Checkbox Group with Grid Columns"
          ...
          <div className="w-full">
            ...
            >
              ...
              <Checkbox
                value="slack"
                label="Slack Hook"
                icon="hugeicons:bubble-chat-delay"
              />
              <Checkbox
                value="webhook"
              ...
              <tbody>
                <tr className="border-b border-border">
                  ...
                  <td className="px-3 py-2 text-muted-foreground">
                    Encloses control inside interactive card container with
                    animated borders.
                  </td>
                </tr>
                <tr className="border-b border-border">
                  ...
                  <td className="px-3 py-2 text-muted-foreground">
                    Content slot rendered at the start of the layout.
                  </td>
                </tr>
                <tr className="border-b border-border">
                  <td className="px-3 py-2 font-mono text-primary">
                  ...
                  <td className="px-3 py-2 text-muted-foreground">—</td>
                  <td className="px-3 py-2 text-muted-foreground">
                    Content slot rendered at the end of the layout.
                  </td>
                </tr>
                <tr className="border-b border-border">
                  <td className="px-3 py-2 font-mono text-primary">
                  ...
                  <td className="px-3 py-2 text-muted-foreground">
                    Placement of the checkbox input root element inside the
                    layout.
                  </td>
                </tr>
                <tr className="border-b border-border">
                  ...
                  <td className="px-3 py-2 text-muted-foreground">false</td>
                  <td className="px-3 py-2 text-muted-foreground">
                    Applies partial selection dash indicator for hierarchical
                    parent items.
                  </td>
                </tr>
                <tr className="border-b border-border">
                  ...
                  <td className="px-3 py-2 text-muted-foreground">—</td>
                  <td className="px-3 py-2 text-muted-foreground">
                    Arranges group checkboxes into grid columns.
                  </td>
                </tr>
                <tr>
                  <td className="px-3 py-2 font-mono text-primary">
                  ...
                  <td className="px-3 py-2 text-muted-foreground">
                    Layout direction for non-grid group items.
                  </td>
                </tr>
              </tbody>
            </table>
