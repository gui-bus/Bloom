...
export function CodeBlock({
  ...
  return (
    <div
      ref={containerRef}
      className={cn("relative bg-[#282a36] text-[#f8f8f2] rounded-3xl p-5 overflow-hidden", className)}
      {...props}
    >
      ...
      <div className="relative mt-4">
        <pre
          className={cn(
            "rounded-2xl text-sm transition-all duration-300 bg-[#282a36] p-3 text-[#f8f8f2]",
        ...
        >
          {highlightedSet.size > 0 ? (
            <div className="font-mono text-sm py-1 bg-[#282a36]">
              {codeLines.map((lineContent, lineIndex) => {
                ...
                return (
                  <div
                    key={lineIndex}
                    className={cn(
                      "px-3 py-0.5 flex items-start gap-4 transition-colors bg-[#282a36]",
                      isHighlighted && "bg-sky-500/20 border-l-2 border-sky-400 font-semibold"
                    )}
                  >
                    <code className={\`language-\${currentLanguage} flex-1 bg-[#282a36]\`}>
                      {lineContent || " "}
                    </code>
                  </div>
                );
            ...
            <code
              ...
              } font-mono bg-[#282a36] text-[#f8f8f2]\`}
            >
              {currentCode}
            </code>
          )}
