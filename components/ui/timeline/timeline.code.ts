export const timelineCode = `"use client";
    ...
    <div className={cn("relative space-y-0", className)}>
      {items.map((child, index) => {
        if (React.isValidElement<TimelineItemProps>(child)) {
          return React.cloneElement(child, { isLast: index === items.length - 1 });
        }
        return child;
      })}
    </div>
...
export function TimelineItem({ title, description, time, icon, status = "default", isLast = false, className }: TimelineItemProps) {
  return (
    <div className={cn("relative flex gap-4 pb-8", isLast && "pb-0", className)}>
      {!isLast && <div className="absolute left-[15px] top-[32px] bottom-0 w-0.5 bg-zinc-200 dark:bg-zinc-800" />}
      <div className={cn("relative z-10 flex items-center justify-center size-8 rounded-full border-2 bg-white dark:bg-zinc-900 shrink-0", statusColors[status])}>
        {icon ? <span className="text-sm">{icon}</span> : <span className={cn("size-2.5 rounded-full", statusDotBg[status])} />}
      </div>
      <div className="flex-1 pt-0.5 min-w-0">
        <div className="flex items-center justify-between gap-3">
          <p className="text-sm font-bold text-zinc-900 dark:text-zinc-100">{title}</p>
          {time && <span className="font-mono text-xs text-zinc-400 shrink-0">{time}</span>}
        </div>
