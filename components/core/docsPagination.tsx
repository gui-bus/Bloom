...
export function DocsPagination() {
  ...
  const currentIndex = docNavigationList.findIndex((item) => item.href === pathname);

  if (currentIndex === -1) return null;

  const prev = currentIndex > 0 ? docNavigationList[currentIndex - 1] : null;
  const next = currentIndex < docNavigationList.length - 1 ? docNavigationList[currentIndex + 1] : null;
    ...
    <div className="pt-8 mt-12 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-4 w-full">
      {prev ? (
        ...
        >
          <span className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider flex items-center gap-1">
            <Icon icon="hugeicons:arrow-left-01" className="size-3.5 group-hover:-translate-x-1 transition-transform" />
            Previous
          </span>
      ...
      {next ? (
        ...
        >
          <span className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider flex items-center gap-1">
            Next
            <Icon icon="hugeicons:arrow-right-01" className="size-3.5 group-hover:translate-x-1 transition-transform" />
          </span>
          <span className="text-sm font-bold text-zinc-900 dark:text-zinc-100 group-hover:text-sky-500 transition-colors">
            {next.label}
          </span>
