# Instalação e Uso via CLI (`@bloomui/cli`)
...
3. **Instalação de Dependências**: O sistema detecta o gerenciador de pacotes ativo no seu projeto (`pnpm`, `npm`, `yarn` ou `bun`) e instala de forma automática todas as dependências adicionais do Radix UI exigidas pelo componente (ex: `@radix-ui/react-slot` para o botão).
